...
/**
 * @classdesc - Does the following:
 * - Creates a directory "external" in the parent directory
 * - Downloads the three external libraries used by this library and creates the corresponding
 *   file in the "external" directory
 * - Iterates the files in the directories "lib" and "src", and replaces the #include statements
 *   with the corresponding relative path
 */
class Main {
    static __New() {
        ...
        this.Items := [
            {
                Url: 'https://github.com/Nich-Cebolla/AutoHotkey-LibV2/raw/refs/heads/main/structs/IntegerArray.ahk'
              , FileName: 'IntegerArray'
            }
          , {
                Url: 'https://github.com/Nich-Cebolla/AutoHotkey-LibV2/raw/refs/heads/main/structs/Logfont.ahk'
              , FileName: 'Logfont'
            }
          , {
    ...
    static Call(encoding := 'UTF-8') {
        parentDir := SubStr(A_LineFile, 1, InStr(A_LineFile, '\', , , -2) - 1)
        dir := parentDir '\external'
        ...
        for item in this.Items {
            ...
            f := FileOpen(dir '\' item.FileName '.ahk', 'w', 'utf-8-raw')
            f.Write(whr.request(item.Url))
            f.Close()
        }
        flag := 0
        for scriptDir in ['lib', 'src'] {
            Loop Files '..\' scriptDir '\*', 'F' {
                content := FileRead(A_LoopFileFullPath, encoding)
                for item in this.Items {
                    content := StrReplace(content, '#include <' item.FileName '>', '#include ' relativeDir '\' item.FileName '.ahk', , &count)
                    if count {
                        flag := 1
                    }
                }
                if flag {
                    f := FileOpen(A_LoopFileFullPath, 'w', encoding)
                    f.Write(content)
                    f.Close()
                    flag := 0
                }
