...
class Win extends RectBase {
    ...
    static MoveByMouse(Hwnd, &OutX?, &OutY?, Params?) {
        ...
        Win.GetPosByMouse(&X, &Y, &mX, &mY, &wW, &wH, Params.OffsetMouse, Params.OffsetEdgeOfMonitor, Unit)
        if MoveImmediately {
    ...
    static ScaleMoveByMouse(Hwnd, UseWorkArea := true, MoveImmediately := true
        ...
        Mon.Win._GetPosByMouse(&x, &y, , OffsetMouse, OffsetEdgeOfMonitor, unitMouse[UseWorkArea ? 'Work' : 'Display'])
        Win.GetPosByMouse(&X, &Y, mX, mY, wW / DpiRatio, wH / DpiRatio, Params.OffsetMouse, Params.OffsetEdgeOfMonitor, Unit)
        if MoveImmediately {
            WinMove(X, Y, , , Number(Hwnd))
        }
        CoordMode('Mouse', oCoordMode)
