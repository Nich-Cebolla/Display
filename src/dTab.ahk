...
class dTab extends Gui.Tab {
     ...
     *
     * @param {VarRef} [OutTabRect] - A variable that will receive the {@link Rect} object representing
    ...
    GetDisplayTopLeft(&OutTabRect?) {
        ...
        OutTabRect := Rect(tabx, taby, tabw + tabx, taby + tabh)
        SendMessage(4904, false, OutTabRect, this.Hwnd, this.Gui.Hwnd) ; TCM_ADJUSTRECT
        return Point(tabx + OutTabRect.l, taby + OutTabRect.t)
    }
    /**
    ...
    SetMinTabWidth(Width) {
        return SendMessage(4913, 0, Width, this.Hwnd, this.Gui.Hwnd) ; TCM_SETMINTABWIDTH
    }
    /**
